...
describe("Date range picker without inputs", () => {
  ...
  it('should throw an error when a toggle button is clicked without a wrapping "cob-date-picker"', () => {
    assert.throws(() => DateRangePicker.on(), {
      message:
        ".cob-date-range-picker is missing inner two '.cob-date-picker' elements",
    });
  });
});
