...
const select = require("../../bwds-core/src/js/utils/select");
const selectOrMatches = require("../../bwds-core/src/js/utils/select-or-matches");
const { prefix: PREFIX } = require("../../bwds-core/src/js/config");
const { CLICK } = require("../../bwds-core/src/js/events");
const activeElement = require("../../bwds-core/src/js/utils/active-element");
const isIosDevice = require("../../bwds-core/src/js/utils/is-ios-device");
const Sanitizer = require("../../bwds-core/src/js/utils/sanitizer");
...
const processFoccobbleSelectors = (...selectors) =>
  selectors.map((query) => query + NOT_DISABLED_SELECTOR).join(", ");

const DATE_PICKER_FOCCOBBLE = processFoccobbleSelectors(
...
const MONTH_PICKER_FOCCOBBLE = processFoccobbleSelectors(
  CALENDAR_MONTH_FOCUSED,
);

const YEAR_PICKER_FOCCOBBLE = processFoccobbleSelectors(
...
const enhanceDatePicker = (el) => {
  ...
  calendarWrapper.insertAdjacentHTML(
    ...
    <div class="cob-sr-only ${DATE_PICKER_STATUS_CLASS}" role="status" aria-live="polite"></div>`,
  );
...
const tabHandler = (foccobble) => {
  const getFoccobbleContext = (el) => {
    const { calendarEl } = getDatePickerContext(el);
    const foccobbleElements = select(foccobble, calendarEl);

    const firstTabIndex = 0;
    const lastTabIndex = foccobbleElements.length - 1;
    const firstTabStop = foccobbleElements[firstTabIndex];
    const lastTabStop = foccobbleElements[lastTabIndex];
    const focusIndex = foccobbleElements.indexOf(activeElement());
  ...
  return {
    tabAhead(event) {
      const { firstTabStop, isLastTab, isNotFound } = getFoccobbleContext(
        event.target,
      );
    ...
    tabBack(event) {
      const { lastTabStop, isFirstTab, isNotFound } = getFoccobbleContext(
        event.target,
      );
...
const datePickerTabEventHandler = tabHandler(DATE_PICKER_FOCCOBBLE);
const monthPickerTabEventHandler = tabHandler(MONTH_PICKER_FOCCOBBLE);
const yearPickerTabEventHandler = tabHandler(YEAR_PICKER_FOCCOBBLE);
