...
const characterCountSelector = () =>
  document.querySelector(".cob-character-count");
const tests = [
...
tests.forEach(({ name, selector: containerSelector }) => {
  describe(`character count component initialized at ${name}`, () => {
    ...
    beforeEach(() => {
      ...
      formGroup = root.querySelector(".cob-form-group");
      label = root.querySelector(".cob-label");
      input = root.querySelector(".cob-character-count__field");
      requirementsMessage = root.querySelector(".cob-character-count__message");
      statusMessageVisual = root.querySelector(".cob-character-count__status");
      statusMessageSR = root.querySelector(".cob-character-count__sr-status");
    });
    ...
    it("hides the requirements hint for screen readers", () => {
      assert.strictEqual(
        requirementsMessage.classList.contains("cob-sr-only"),
        true,
      );
    });
    ...
    it("creates a screen reader status message on init", () => {
      const srStatus = document.querySelectorAll(
        ".cob-character-count__sr-status",
      );
